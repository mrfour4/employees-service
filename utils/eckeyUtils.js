let ecKeyUtils = (() => {
    ...
    const oidToCurve = {};
    for (let [k, v] of Object.entries(curveToOid)) oidToCurve[v] = k;

    // generate a reverse map for oidedToAnsi
    const ansiToOided = {};
    for (let [k, v] of Object.entries(oidedToAnsi)) ansiToOided[v] = k;
    ...
    function encodeSkToDer(crve, sk, pk) {
        ...
        // version        INTEGER { ecPrivkeyVer1(1) } (ecPrivkeyVer1)
        let seqe = Buffer.concat([
            Buffer.from("020101", "hex"),
            ske,
            ecpe,
            pke,
        ]); // version: tag of INTEGER | length = 1 | value = 1
    ...
    function encodePkToDer(crve, pk) {
        ...
        /*
          ...
          */
        let alge = Buffer.concat([
            Buffer.from([0x30]),
            encodeBuf(Buffer.concat([id_ecPublicKey, crve])),
        ]); // tag of SEQUENCE
        ...
        return Buffer.concat([
            Buffer.from([0x30]),
            encodeBuf(Buffer.concat([alge, pke])),
        ]); // tag of SEQUENCE
    }
    ...
    return {
        generateDer: (arg1, arg2) => {
            ...
            if (!cname) throw Error("Curve name is not optional");
            let crve = curveToOid[cname];
            if (crve === null) throw Error("Unsupported elliptic curve");

            if (!sk && !pk)
                throw Error("Either privateKey or publicKey is required");

            if (
                (sk && !(sk instanceof Buffer)) ||
                (pk && !(pk instanceof Buffer))
            )
        ...
        parsePem: (pemContent) => {
            ...
            // DER encoding of SubjectPublicKeyInfo
                ...
                (s =
                    /^\-\-\-\-\-BEGIN PUBLIC KEY\-\-\-\-\-\n([^]+)\n\-\-\-\-\-END PUBLIC KEY\-\-\-\-\-$/g.exec(
                        pemContent
                    ))
            )
                return parseDer$Spki(Buffer.from(s[1], "base64"));
            else throw Error("Invalid or unsupported PEM content");
        },
        ...
        generateJwk: (arg1, arg2) => {
            ...
            if (!ansiName)
                throw Error(
                    "JWK doesn't support curves outside of P-256/384/521"
                );
            ...
            if (sk && pk) {
                let privateKey = {
                    kty: "EC",
                    crv: ansiName,
                    key_ops: ["deriveBits", "deriveKey", "sign"],
                    kid: "JWK-sk generated by ecKeyUtils",
                };
                if (pk[0] !== 0x04)
                    throw Error("Only support uncompressed public key");

                let x = pk.slice(1, (pk.length - 1) / 2 + 1),
                    y = pk.slice(-(pk.length - 1) / 2);
            ...
            if (pk) {
                let publicKey = {
                    ...
                    crv: ansiName,
                    key_ops: ["verify"],
                    kid: "JWK-pk generated by ecKeyUtils",
                };
                if (pk[0] !== 0x04)
                    throw Error("Only support uncompressed public key");
        ...
        parseJwk: (jwk) => {
            if (jwk.kty !== "EC") throw Error("Not an EC JSON Web Key");
